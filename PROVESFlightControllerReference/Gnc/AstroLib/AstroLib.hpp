// ======================================================================
// \file   AstroLib.hpp
// \brief  Framework-independent astrodynamics kernel.
//
// Deliberately contains ZERO F Prime and ZERO Zephyr dependencies so it
// can be compiled and unit-tested on a host against Astropy/Skyfield
// truth data. The F Prime component is a thin shell around this.
//
// Conventions:
//   - Distances in km, angles in radians unless suffixed _deg / Au.
//   - Frames are "of date": MOD / TOD / TEME / ECEF. See README.
...
//! Normalize. Returns the input unchanged if it is (near) zero length,
//! so callers never see NaN propagate into the attitude chain.
Vec3 vunit(const Vec3& a);

//! Clamp to [-1, 1] before acos/asin. Guards against domain errors from
//! rounding, which on a soft-float target are more likely than you think.
...
//! A Julian date carried as two doubles (whole day + fraction) so that we
//! keep ~microsecond resolution instead of the ~50 us a single double
//! gives you at JD 2.46e6.
struct JulianDate2 {
